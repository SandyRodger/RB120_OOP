class Board
  ...
  def draw
	...
	def find_at_risk_square(line, board)
		if board.squares.values_at(*line).count(PLAYER_MARKER) == 2
...
class TTTGame
  ...
  def initialize
    ...
    @current_marker = FIRST_TO_MOVE
		@human_wins = 0
		@computer_wins = 0
		@human_five_wins = false
		@computer_five_wins = false
  end
  ...
  private
	...
	def five_wins_message
		if @human_five_wins
			puts "You've won the best of 5! Congratulations."
		elsif @computer_five_wins
			puts "The computer has won the best of 5. Better luck next time!"
		end
	end
	...
	def tally_score(marker)
		...
		when HUMAN_MARKER
			@human_wins += 1
			if @human_wins == 5
				@human_five_wins = true
			end
		when COMPUTER_MARKER
			@computer_wins += 1
			if @computer_wins == 5
				@computer_five_wins = true
			end
		end
  ...
  def computer_moves
		board.computer_places_piece!(board)
  end
